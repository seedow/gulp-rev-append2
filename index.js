...
var revPlugin = function revPlugin(opts) {
  return map(function(file, cb) {
    ...
    var resourcePath = opts ? opts.resourcePath || undefined : undefined;
    var contents;
    var lines;
    var i, length;
    var line;
    ...
    for (i = 0; i < length; i++) {
      ...
      if (groups && groups.length > 1) {
        ...
        //If the resource path is provided overwrite dependecyPath
        if (resourcePath) {
          dependencyPath = path.resolve(path.join(file.base, resourcePath,normPath));
        }
        try {
